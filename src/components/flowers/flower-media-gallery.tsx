...
import { FlowerMedia, Flower } from "@/lib/repositories/repository-types";
...
export const getValidImageUrl = (mediaItem: FlowerMedia) => {
  if (!mediaItem) return "";
...
export const getVideoPoster = (mediaItem: FlowerMedia) => {
  ...
  if (mediaItem.id) {
    return `/api/media/thumbnail?id=${mediaItem.id}&type=flower`;
  }
...
export function Lightbox({ flower, onClose, initialIndex = 0 }: { flower: Flower; onClose: () => void; initialIndex?: number }) {
  const [currentIndex, setCurrentIndex] = useState(initialIndex);
  const media = flower.media || [];
...
// FlowerMediaGallery component for displaying flower images in a carousel
interface FlowerMediaGalleryProps {
  media: FlowerMedia[];
  alt: string;
...
export function FlowerMediaGallery({ media, alt, onImageClick, height = "h-48" }: FlowerMediaGalleryProps) {
      ...
      <div className={`relative w-full ${height} overflow-hidden group cursor-pointer`} onClick={onImageClick}>
        <div className="relative w-full h-full transition-transform duration-300 group-hover:scale-105">
          <Image 
            ...
            alt={alt || media[0].file_name || "Flower image"}
            fill
      ...
      >
        <div className="relative w-full h-full">
          <Image 
            ...
            alt={alt || media[currentIndex].file_name || "Flower image"}
            fill
            className="object-cover"
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
            priority={true}
